    ...
    def so(u):
        ...
        for meta in soup.find_all("meta"):
            ...
            if"category"in nm:
                ...
                break
        return{"name":d.get("name",""),"description":d.get("description",""),"html":html,
...
def extract_aliexpress(url, shared_page=None, pause_event=None):
    from AliExpress import AliExpress;from bs4 import BeautifulSoup
    if pause_event:pause_event.wait()
    try:
        ae=AliExpress(url,external_page=shared_page)
        d=ae.scrape()
        if not d:return[]
        pd=os.path.join(os.path.dirname(__file__),"Outputs",d.get("product_name_safe","Unknown"))
        pp=os.path.join(pd,"page.html")
        if not os.path.exists(pp):
            # Fallback: basic data without page.html
            sku_match=re.search(r'/item/(\d+)\.html',url)
            if not sku_match:sku_match=re.search(r'(\d{10,})',url)
            ps=sku_match.group(1)if sku_match else"unknown"
            images=[]
            pval=f"{d.get('current_price_integer','')}.{d.get('current_price_decimal','00')}"if d.get('current_price_integer')else""
            return[{"parent_sku":ps,"name":d.get("name",""),"url":url,"price":pval,"discount":d.get("discount_percentage",""),
                    "description":d.get("description",""),"color":"","sizes":["One Size"],"images":images,
                    "variant_images":[],"shop":"AliExpress","skus":[],"color_images":{}}]
        with open(pp,'r',encoding='utf-8')as f:html=f.read()
        soup=BeautifulSoup(html,"html.parser")
        # Extract from rendered HTML: gallery images + variant selectors
        title=d.get("name","")
        # Gallery images
        images=[]
        gallery=soup.find("div",{"class":"slider--wrap--dfLgmYD"})
        if gallery:
            for img in(gallery.find_all("img")if gallery else[]):
                src=img.get("src")or img.get("data-src")or""
                if src:
                    if src.startswith('//'):src='https:'+src
                    src=re.sub(r'_\d{2,4}x\d{2,4}','_960x960',src)
                    if src not in images:images.append(src)
        # Variant selectors: look for sku-property elements in rendered DOM
        colors=[];sizes=[];vi=[];ci={}
        for prop_el in soup.select('[class*="sku-property"]'):
            prop_name_el=prop_el.select_one('[class*="sku-property-title"], [class*="sku-title"]')
            prop_name=prop_name_el.text.strip()if prop_name_el else""
            for item in prop_el.select('li, [class*="sku-property-item"], [class*="property-item"]'):
                img_el=item.find('img')
                img_src=img_el.get('src')or img_el.get('data-src')or''if img_el else''
                if img_src:
                    if img_src.startswith('//'):img_src='https:'+img_src
                    vi.append(img_src)
                val_name=item.get('title','')or item.text.strip()
                if prop_name and'color' in prop_name.lower()or'색' in prop_name:
                    if val_name not in colors:colors.append(val_name)
                    if img_src and val_name not in ci:ci[val_name]=img_src
                else:
                    if val_name not in sizes:sizes.append(val_name)
        if not sizes:sizes=["One Size"]
        price=f"{d.get('current_price_integer','')}.{d.get('current_price_decimal','00')}"if d.get('current_price_integer')else""
        sku_match=re.search(r'/item/(\d+)\.html',url)
        if not sku_match:sku_match=re.search(r'(\d{10,})',url)
        ps=sku_match.group(1)if sku_match else"unknown"
        if colors:
            return[{"parent_sku":ps,"name":title,"url":url,"price":price,"discount":d.get("discount_percentage",""),
                    "description":d.get("description",""),"color":c,"sizes":sizes,"images":images[:20],
                    "variant_images":vi,"shop":"AliExpress","skus":[],
                    "color_images":ci}for c in colors]
        return[{"parent_sku":ps,"name":title,"url":url,"price":price,"discount":d.get("discount_percentage",""),
                "description":d.get("description",""),"color":"","sizes":sizes,"images":images[:20],
                "variant_images":vi,"shop":"AliExpress","skus":[],"color_images":ci}]
    except Exception as e:
...
class App:
    ...
    def __init__(self):
        ...
        # === Two-tab layout ===
        nb=ttk.Notebook(self.rt);nb.pack(fill=tk.BOTH,expand=True,padx=20,pady=(5,0))
        ...
        cols=("name","color","price","discount","sizes","imgs")
        self.tv=ttk.Treeview(t1,columns=cols,show="headings",height=8)
        for cid,ch,cw in zip(cols,["商品名称","颜色","价格","折扣","尺码","图片"],[370,70,65,50,160,50]):
            self.tv.heading(cid,text=ch);self.tv.column(cid,width=cw)
        self.tv.pack(fill=tk.BOTH,expand=True,padx=10,pady=2)
        ...
        # === Shared Log ===
        tk.Label(self.rt,text="运行日志",font=("Arial",9,"bold"),bg=self.BG,fg=self.T).pack(anchor=tk.W,padx=20)
        self.lg=scrolledtext.ScrolledText(self.rt,height=4,font=("Consolas",9),bg="#ECEFF1",fg="#37474F",relief="solid",bd=1)
        self.lg.pack(fill=tk.X,padx=20,pady=(0,8))
        self.products=[];self._urls=[];self._url_idx=0
        self.rt.after(200,self._load_session)

    def _lm(self,m):self.lg.insert(tk.END,f"[{datetime.now().strftime('%H:%M:%S')}] {m}\n");self.lg.see(tk.END);self.rt.update_idletasks()
    def _su(self):
    ...
    def _load_session(self):
        ...
        try:
            ...
            for r in data.get("tv_rows",[]):
                self.tv.insert("","end",values=[r.get(k,"")for k in["name","color","price","discount","sizes","imgs"]])
        ...
        try:
            ...
            for i,url in enumerate(urls,1):
                ...
                try:
                    ...
                    while cw<60:
                        cu=page.url
                        if "challenge" not in cu and "risk" not in cu:
                            break
    ...
    def _resume(self):
        ...
        # If no active thread (restored from session), start new with saved position
        if not hasattr(self,'_pause_event')or self._pause_event.is_set():
            t=self.ut.get("1.0",tk.END).strip()
                ...
                try:
                    ...
                    if prods:
                        for p in prods:
                            ...
                            self.tv.insert("",tk.END,values=(p.get("name","")[:50],p.get("color",""),p.get("price",""),p.get("discount",""),sz,len(p.get("images",[]))))
                            self._lm(f"  [{p.get('color','?')}] {p.get('price','?')} | {len(p.get('sizes',[]))} sizes | {len(p.get('images',[]))} imgs")
        ...
        try:
            ...
            for p in self.products:
                ...
                for size in p.get("sizes",["One Size"]):
                    ...
                    ws.cell(row=row,column=5,value=p.get("shein_category",""))
                    ...
                    row+=1
            wb.save(fp);self._lm(f"Excel: {fp} ({row-2} rows)");messagebox.showinfo("Done",f"Exported {row-2} rows")
